...
class RepositoryAnalyzer:
    ...
    def _detect_project_types(self, file_tree: List[FileInfo], analysis: Dict) -> List[ProjectTypeResult]:
        ...
        # Next.js Detection
        nextjs_result = self._detect_nextjs_project(file_names, file_paths, analysis)
        if nextjs_result:
            project_types.append(nextjs_result)
    ...
    def _detect_nextjs_project(self, file_names: List[str], file_paths: List[str], analysis: Dict) -> Optional[ProjectTypeResult]:
        """Detect Next.js projects"""
        evidence = []
        confidence = 0.0
        
        if 'next.config.js' in file_names or 'next.config.mjs' in file_names:
            evidence.append('Next.js config found')
            confidence += 50
        
        if any(path.startswith('pages/') for path in file_paths):
            evidence.append('Next.js pages directory detected')
            confidence += 30
        
        if any(path.startswith('app/') for path in file_paths):
            evidence.append('Next.js app directory detected')
            confidence += 35

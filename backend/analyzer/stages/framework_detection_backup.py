"""
🎯 Framework Detection Stage - Real Implementation
Intelligent detection of web frameworks, static sites, and application stacks
"""
import logging
...
class FrameworkDetectionStage:
    ...
    def _detect_monorepo(self, repo_path: Path) -> Dict[str, Any]:
        ...
        if pnpm_workspace.exists():
            ...
            try:
                import yaml
                workspace_data = yaml.safe_load(pnpm_workspace.read_text(encoding="utf-8"))
                if "packages" in workspace_data:
                    for pattern in workspace_data["packages"]:
                        self._find_workspace_packages(repo_path, pattern, monorepo_info["packages"])
            except Exception as e:
                logger.debug(f"Could not parse pnpm-workspace.yaml: {e}")
        ...
        if lerna_json.exists():
            ...
            try:
                lerna_data = json.loads(lerna_json.read_text(encoding="utf-8"))
                if "packages" in lerna_data:
                    for pattern in lerna_data["packages"]:
                        self._find_workspace_packages(repo_path, pattern, monorepo_info["packages"])
            except Exception as e:
                logger.debug(f"Could not parse lerna.json: {e}")
    ...
    def _detect_toolkit(self, repo_path: Path, monorepo_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # CRA Detection
        cra_signals = [
            (repo_path / "packages" / "react-scripts").exists(),
            (repo_path / "packages" / "create-react-app").exists(),
            any((repo_path / "packages").glob("cra-template*")) if (repo_path / "packages").exists() else False,
            any((repo_path / "packages").glob("babel-preset-react-app")) if (repo_path / "packages").exists() else False,
        ]
        ...
        if any(cra_signals):
            ...
            # Find CRA templates
            if (repo_path / "packages").exists():
                for template_dir in (repo_path / "packages").glob("cra-template*"):
            ...
            return {
                ...
                "framework": {
                    "name": "cra-tooling",
                    "confidence": 0.99,
                    "evidence": [f"packages/{p.name}" for p in (repo_path / "packages").iterdir() 
                               if p.name in ["react-scripts", "create-react-app"] or p.name.startswith("cra-template")]
                },
                "templates": templates
            }
    ...
    def _detect_frameworks_in_path(self, path: Path) -> List[Dict[str, Any]]:
        ...
        return frameworks"🔍 Intelligent Framework Detection with High-Signal Rules"""

    async def analyze(self, context) -> None:
        repo_path = getattr(context, "repo_path", None)
        if not repo_path or not repo_path.exists():
            logger.error(f"Repository path does not exist: {repo_path}")
            # ensure intelligence_profile exists and is a dict
        ...
        # ✅ Normalize context fields to avoid `.get` on non-dicts
        ...
        # (you don't use file_intelligence here – safe to drop, but keep if needed)
        file_intelligence = context.intelligence_profile.get("file_intelligence", {})
        ...
        # 1) Static first (now tolerant of incidental PHP)
